...
import { CheckCircle, Clock, Award, BookOpen, BarChart3, Play, RotateCcw, Moon, Sun, XCircle } from "lucide-react";
import { cn } from "@/lib/utils";

// Types --------------------------------------------------------------------
interface GH300Topic {
  name: string;
  description: string;
...
export default function QuizApp() {
  ...
  const [mode, setMode] = useState<"home" | "quiz" | "review" | "results" | "coverage" | "gh300-topics">("home");
  ...
  const progress = Math.round(filtered.length > 0 ? ((currentIdx + 1) / filtered.length) * 100 : 0);
  ...
  // Coverage and topic calculations ---------------------------------------------------
  const { coverage, topics } = useMemo(() => {
    ...
    // Calculate topics from domains
    const topics: GH300Topic[] = Object.entries(byDom).map(([domain, count]) => ({
      name: domain,
      description: `Questions related to ${domain}`,
      questionCount: count,
      completionRate: history.filter(h => h.q.domain === domain && h.ok).length / count * 100 || 0
    }));

    return { 
      coverage: { total, byDom, byDiff },
      topics 
    };
  }, [allQs, history]);
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <BadgeWrapper variant="outline">{current.domain}</BadgeWrapper>
                <BadgeWrapper className={getDifficultyColor(current.difficulty)}>
                  {current.difficulty}
                </BadgeWrapper>
                {current.type === "multi" && (
                  <BadgeWrapper variant="secondary">Multiple Selection</BadgeWrapper>
                )}
              </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="space-x-2">
                <BadgeWrapper variant="outline">{item.q.domain}</BadgeWrapper>
                <BadgeWrapper className={getDifficultyColor(item.q.difficulty)}>
                  {item.q.difficulty}
                </BadgeWrapper>
              </div>
              {item.ok ? (
              ...
              <div className="space-y-3">
                {Object.entries(coverage.byDom).map(([domain, count]) => (
                  <div key={domain} className="space-y-1">
                    ...
                    <Progress value={Math.round((count / (coverage.total || 1)) * 100)} className="h-2" />
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {Object.entries(coverage.byDiff).map(([difficulty, count]) => (
                  <div key={difficulty} className="space-y-1">
                    ...
                    <Progress value={Number((count / coverage.total || 0) * 100)} className="h-2" />
                  </div>
                ))}
              </div>
            </div>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-4">
              <h3 className="font-medium">Core Exam Topics</h3>
                  ...
                  <Card key={index} className="hover:bg-accent/5 transition-colors">
                    <CardContent className="p-4">
                      ...
                      <div className="mt-4 flex items-center gap-2">
                        ...
                        <BadgeWrapper className={topic.completionRate >= 70 ? "bg-success" : "bg-warning"}>
                          {Math.round(topic.completionRate)}% mastery
                        </BadgeWrapper>
                      </div>
                    </CardContent>
                  </Card>
        ...
        {mode === "home" && (
          <Tabs defaultValue="home" className="space-y-6">
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="gh300-topics">GH300 Topics</TabsTrigger>
            </TabsList>
            <TabsContent value="home">{renderHome()}</TabsContent>
            <TabsContent value="coverage">{renderCoverage()}</TabsContent>
            <TabsContent value="gh300-topics">{renderGH300Topics()}</TabsContent>
          </Tabs>
        )}
